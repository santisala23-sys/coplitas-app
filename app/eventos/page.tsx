...
export default function EventosPage() {
  ...
  const toggleMaterial = (mat: any) => {
    const existe = formEvento.materiales.find(m => m.id === mat.id)
    if (existe) {
      setFormEvento({ ...formEvento, materiales: formEvento.materiales.filter(m => m.id !== mat.id) })
    } else {
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-5 border-b flex justify-between items-center bg-white"><h2 className="text-xl font-bold text-rose-800">{eventoEditando ? 'Editar Evento' : 'Nuevo Evento'}</h2><button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:bg-gray-100 rounded-full font-bold text-xl p-2 w-10 h-10">✕</button></div>
            
            <form onSubmit={handleSaveEvento} className="p-6 flex flex-col gap-4 overflow-y-auto">
              
              <div><label className="text-sm font-semibold text-gray-700">Título del Evento *</label><input required value={formEvento.titulo} onChange={e => setFormEvento({...formEvento, titulo: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" placeholder="Ej: Cumpleaños de Sofía" /></div>
              
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-sm font-semibold text-gray-700">Fecha *</label><input required type="date" value={formEvento.fecha} onChange={e => setFormEvento({...formEvento, fecha: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" /></div>
                <div><label className="text-sm font-semibold text-gray-700">Hora</label><input type="time" value={formEvento.hora} onChange={e => setFormEvento({...formEvento, hora: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" /></div>
              </div>

              <div><label className="text-sm font-semibold text-gray-700">Dirección</label><input value={formEvento.direccion} onChange={e => setFormEvento({...formEvento, direccion: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" placeholder="Ej: Salón La Fiesta, Av. San Martín 123" /></div>
              
              <div>
                <label className="text-sm font-semibold text-gray-700">Asignar a:</label>
                <select value={formEvento.usuario_asignado} onChange={e => setFormEvento({...formEvento, usuario_asignado: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1 capitalize">
              ...
              <div>
                ...
                <div className="border rounded-xl p-3 bg-gray-50 max-h-32 overflow-y-auto flex flex-wrap gap-2">
                  {materiales.map(mat => {
                    const seleccionado = formEvento.materiales.some(m => m.id === mat.id)
                    return (
              ...
              <div><label className="text-sm font-semibold text-gray-700">Notas Especiales</label><textarea value={formEvento.notas} onChange={e => setFormEvento({...formEvento, notas: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1 resize-none" rows={2} placeholder="Ej: Hay que llegar 15 min antes, llevar pendrive." /></div>

              <div className="mt-4 flex gap-3 pt-2 border-t">
                <button type="button" onClick={() => setIsModalOpen(false)} className="w-1/3 py-3 rounded-xl font-medium text-gray-600 bg-gray-100 hover:bg-gray-200">Cancelar</button>
                <button type="submit" disabled={isSaving || !formEvento.titulo || !formEvento.fecha} className="w-2/3 py-3 rounded-xl font-bold bg-rose-600 text-white hover:bg-rose-700 disabled:opacity-50">
