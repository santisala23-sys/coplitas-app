...
export default function SedesPage() {
  ...
  useEffect(() => {
    // Si no es ADMIN, lo pateamos al home para que no espíe
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto min-h-screen">
      ...
      <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-4">
        ...
        <button onClick={() => { setSedeEditando(null); setFormSede({nombre: '', estado: 'ACTIVA'}); setIsSedeModalOpen(true); }} className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-3 rounded-xl font-semibold shadow-sm transition w-full md:w-auto">
          + Nueva Sede
        </button>
      </div>
          ...
          {sedes.map((sede) => (
            <div key={sede.id} className="bg-white p-5 rounded-2xl shadow-sm border border-teal-100 flex items-center justify-between">
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => { setSedeEditando(sede); setFormSede({nombre: sede.nombre, estado: sede.estado}); setIsSedeModalOpen(true); }} className="text-blue-500 hover:bg-blue-50 p-2 rounded-lg transition font-medium">Editar</button>
