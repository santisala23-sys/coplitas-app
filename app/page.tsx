...
export default function HomePage() {
  ...
  useEffect(() => {
    async function fetchStats() {
      // Pedimos a Supabase que cuente la cantidad de canciones
      const { count: cancionesCount } = await supabase
        .from('canciones')
        .select('*', { count: 'exact', head: true })
      
      // Pedimos a Supabase que cuente la cantidad de planificaciones
      const { count: planisCount } = await supabase
        .from('planificaciones')
        .select('*', { count: 'exact', head: true })

      // Leemos el usuario actual para contar SUS tareas pendientes
          ...
          <div className="bg-purple-100/50 w-full py-3 rounded-xl">
            {loading ? (
              ...
              <p className="text-purple-700 font-medium">
                <strong className="text-lg">{stats.canciones}</strong> canciones cargadas
              </p>
            )}
          </div>
        </Link>
          ...
          <div className="bg-emerald-100/50 w-full py-3 rounded-xl">
            {loading ? (
              ...
              <p className="text-emerald-700 font-medium">
                <strong className="text-lg">{stats.planis}</strong> planis armadas
              </p>
            )}
          </div>
        </Link>
          ...
          <div className={`${stats.tareas > 0 ? 'bg-orange-100' : 'bg-gray-100'} w-full py-3 rounded-xl transition-colors`}>
            {loading ? (
              ...
              <p className={`${stats.tareas > 0 ? 'text-orange-700' : 'text-gray-500'} font-medium`}>
                {stats.tareas > 0 ? (
                  <>Tenés <strong className="text-lg">{stats.tareas}</strong> tarea(s) pendiente(s)</>
                ) : (
