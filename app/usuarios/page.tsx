...
export default function UsuariosPage() {
  ...
  useEffect(() => {
    // Si no es ADMIN, lo pateamos al home
  ...
  const handleSaveUsuario = async (e: React.FormEvent) => {
    ...
    try {
      if (usuarioEditando) {
        ...
        // Si llenaron la contraseña, la actualizamos. Si la dejaron vacía, mantenemos la vieja.
        ...
        // Validar que el username no exista ya en la tabla
      ...
      setFormUsuario({ username: '', password: '', role: 'USER', puesto: '', activo: true })
              ...
              <div className="flex gap-2 shrink-0">
                <button 
                  onClick={() => { 
                    ...
                    setFormUsuario({username: u.username, password: '', role: u.role, puesto: u.puesto || '', activo: u.activo}); 
                    setErrorMsg('');
                    setIsModalOpen(true); 
                  }} 
          ...
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSaveUsuario} className="p-6 flex flex-col gap-4 overflow-y-auto">
              ...
              <div>
                ...
                <input 
                  required 
                  disabled={!!usuarioEditando} // No dejamos cambiar el username una vez creado para no romper tareas asociadas
              ...
              <div>
                <label className="text-sm font-semibold text-gray-700">
                  Contraseña {usuarioEditando && <span className="font-normal text-gray-400">(Dejar en blanco para no cambiarla)</span>}
                </label>
                <input 
                  type="password"
              ...
              <div>
                <label className="text-sm font-semibold text-gray-700">Puesto o Rol (Opcional)</label>
                <input 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-semibold text-gray-700">Nivel de Acceso *</label>
                  <select 
                    value={formUsuario.role} 
                    onChange={e => setFormUsuario({...formUsuario, role: e.target.value})} 
                    className="w-full p-3 border rounded-xl bg-gray-50 mt-1 focus:ring-2 focus:ring-purple-500 outline-none"
                  >
                    <option value="USER">Usuario (Solo lectura/tareas)</option>
