...
export default function InventarioPage() {
  ...
  // Formularios
  const [formMaterial, setFormMaterial] = useState({ nombre: '', cantidad_total: 1, descripcion: '' })
  ...
  const fetchData = async () => {
    ...
    // Traer historial de movimientos (uniendo el nombre del material)
  ...
  // --- GUARDAR NUEVO MATERIAL (Solo Admin) ---
  const handleSaveMaterial = async (e: React.FormEvent) => {
    e.preventDefault()
    setIsSaving(true)
    await supabase.from('materiales').insert([formMaterial])
    setFormMaterial({ nombre: '', cantidad_total: 1, descripcion: '' })
  ...
  const handleSaveMovimiento = async (e: React.FormEvent) => {
    ...
    setIsSaving(false)
    fetchData()
    setActiveTab('historial') // Lo mandamos a ver su movimiento recién hecho
  }

  const abrirModalMovimiento = (material: any) => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto min-h-screen">
      ...
      <div className="flex gap-4 md:gap-8 mb-8 border-b-2 border-gray-200">
        <button 
          className={`pb-3 text-lg font-bold px-2 ${activeTab === 'stock' ? 'text-blue-600 border-b-4 border-blue-600 -mb-0.5' : 'text-gray-400 hover:text-gray-600'}`}
          onClick={() => setActiveTab('stock')}
        >
          Stock General
        </button>
        <button 
          className={`pb-3 text-lg font-bold px-2 ${activeTab === 'historial' ? 'text-blue-600 border-b-4 border-blue-600 -mb-0.5' : 'text-gray-400 hover:text-gray-600'}`}
      ...
      {loading ? (
        ...
        <>
                ...
                <div key={mat.id} className="bg-white p-5 rounded-2xl shadow-sm border border-blue-100 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="bg-blue-50 text-blue-700 font-bold px-3 py-1 rounded-lg shrink-0">
                        Total: {mat.cantidad_total}
                      </span>
                    </div>
                  ...
                  <button 
                    onClick={() => abrirModalMovimiento(mat)}
                    className="mt-4 w-full bg-blue-50 text-blue-700 font-semibold py-2.5 rounded-xl border border-blue-200 hover:bg-blue-100 transition"
                  >
                    Mover Material 📦
                  </button>
                </div>
          ...
          {/* VISTA 2: HISTORIAL (LINEA DE TIEMPO) */}
          {activeTab === 'historial' && (
            <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-300 before:to-transparent">
              {movimientos.map((mov) => (
                <div key={mov.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white bg-blue-100 text-blue-600 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 text-xl">
                    📦
                  </div>
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                    ...
                    <p className="text-sm text-gray-600 mb-2">
                      Movió <strong className="text-blue-600">{mov.cantidad} {mov.material?.nombre}</strong>
                    </p>
      ...
      {/* --- MODAL NUEVO MATERIAL (ADMIN) --- */}
      {isMaterialModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="p-5 border-b flex justify-between items-center"><h2 className="text-xl font-bold">Nuevo Material</h2><button onClick={() => setIsMaterialModalOpen(false)} className="text-gray-400 font-bold text-xl p-2">✕</button></div>
            <form onSubmit={handleSaveMaterial} className="p-6 flex flex-col gap-4">
              <div><label className="text-sm font-semibold text-gray-700">Nombre *</label><input required value={formMaterial.nombre} onChange={e => setFormMaterial({...formMaterial, nombre: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" placeholder="Ej: Pañuelos de Colores" /></div>
              <div><label className="text-sm font-semibold text-gray-700">Cantidad Total Existente *</label><input required type="number" min="1" value={formMaterial.cantidad_total} onChange={e => setFormMaterial({...formMaterial, cantidad_total: parseInt(e.target.value)})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" /></div>
              <div><label className="text-sm font-semibold text-gray-700">Descripción / Detalles</label><input value={formMaterial.descripcion} onChange={e => setFormMaterial({...formMaterial, descripcion: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" placeholder="Ej: Están en la caja roja" /></div>
              <div className="mt-4 flex gap-3"><button type="button" onClick={() => setIsMaterialModalOpen(false)} className="w-1/3 py-3 rounded-xl font-medium text-gray-600 bg-gray-100">Cancelar</button><button type="submit" disabled={isSaving} className="w-2/3 py-3 rounded-xl font-bold bg-blue-600 text-white disabled:opacity-50">{isSaving ? 'Guardando...' : 'Guardar'}</button></div>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSaveMovimiento} className="p-6 flex flex-col gap-4">
              <div><label className="text-sm font-semibold text-gray-700">¿Cuántos vas a mover? (Max: {materialSeleccionado.cantidad_total}) *</label><input required type="number" min="1" max={materialSeleccionado.cantidad_total} value={formMovimiento.cantidad} onChange={e => setFormMovimiento({...formMovimiento, cantidad: parseInt(e.target.value)})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1 text-lg font-bold text-blue-700" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-sm font-semibold text-gray-700">Origen *</label><input required value={formMovimiento.origen} onChange={e => setFormMovimiento({...formMovimiento, origen: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" placeholder="Ej: Depósito" /></div>
                <div><label className="text-sm font-semibold text-gray-700">Destino *</label><input required value={formMovimiento.destino} onChange={e => setFormMovimiento({...formMovimiento, destino: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" placeholder="Ej: Sede Palermo" /></div>
              </div>
              <div><label className="text-sm font-semibold text-gray-700">Aclaraciones (Opcional)</label><input value={formMovimiento.notas} onChange={e => setFormMovimiento({...formMovimiento, notas: e.target.value})} className="w-full p-3 border rounded-xl bg-gray-50 mt-1" placeholder="Ej: Faltó uno que estaba roto" /></div>
